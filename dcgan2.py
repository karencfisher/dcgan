...
class DCGAN:
    ...
    def train(self, X, num_epochs, batch_size, verbose=5, checkpoint_path=None):
        ...
        for epoch in range(num_epochs):
            ...
            for _ in tqdm(range(batch_count), ascii=True, desc=f'Epoch {epoch+1}'):
                ...
                noise = np.random.normal(0, sigma, size=(batch_size, 1, 1, self.latent_dim))
            ...
            if verbose and (epoch == 0 or(epoch + 1) % verbose == 0):
                self.generate(10, sigma, epoch=epoch+1, display=True)
    ...
    def generate(self, n_examples, sigma, epoch=None, display=False):
        noise = np.random.normal(0, sigma, size=(n_examples, 1, 1, self.latent_dim))
